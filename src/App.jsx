...
async function saveCharToCloud(user,char){
  ...
  await supabase.from('characters').upsert({id:user.id,data:char,updated_at:new Date().toISOString()});
}
...
const TDEFS={
  w:{bg:"#090912",border:"#16162a",passable:false},
  ".":{bg:"#14142a",passable:true},
  c:{bg:"#1f0a2e",passable:false,icon:"🎵",label:"CLUB",action:"club",col:"#ff4da6"},
  i:{bg:"#0a1f1f",passable:false,icon:"🍺",label:"溜まり場",action:"inn",col:"#00e5ff"},
  s:{bg:"#1f1f0a",passable:false,icon:"🛍",label:"SHOP",action:"shop",col:"#b3ff00"},
  t:{bg:"#0a1f0a",passable:false,icon:"💪",label:"練習場",action:"train",col:"#ffd60a"},
  n:{bg:"#1a1428",passable:false,icon:"💬",action:"npc",col:"#ce93d8"},
  X:{bg:"#0a1428",passable:true,icon:"🚪",action:"exit",col:"#80c0ff"},
};
function getTemplate(lv){return lv>=22?"large":lv>=10?"medium":"small";}
function startPos(rows){
  for(let y=rows.length-1;y>=0;y--){const x=rows[y].indexOf("X");if(x!==-1)return{x,y:y-1};}
  return{x:8,y:5};
}

/* ── D-PAD ── */
function DPad({onMove}){
  const bs={width:46,height:46,borderRadius:8,background:"rgba(255,255,255,0.12)",border:"1px solid rgba(255,255,255,0.2)",color:"#fff",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",userSelect:"none",WebkitUserSelect:"none",touchAction:"none"};
  const h=dir=>e=>{e.preventDefault();onMove(dir);};
  return(
    <div style={{position:"fixed",bottom:96,right:16,display:"grid",gridTemplateColumns:"46px 46px 46px",gap:4,zIndex:200}}>
      <div/><button style={bs} onPointerDown={h("up")}>▲</button><div/>
      <button style={bs} onPointerDown={h("left")}>◀</button>
      <div style={{...bs,background:"rgba(255,255,255,0.04)",cursor:"default"}}/>
...
/* ── INN VIEW (溜まり場) ── */
function InnView({city,char,setChar,genre,onBack,pushNotif}){
  const[players,setPlayers]=useState([]);
  const[loading,setLoading]=useState(false);
  useEffect(()=>{
    if(!supabase)return;
    setLoading(true);
    supabase.from("dancers").select("name,genre,power,level,hometown")
      .neq("name",char.name).order("power",{ascending:false}).limit(12)
      .then(({data})=>{setPlayers(data||[]);setLoading(false);})
      .catch(()=>setLoading(false));
  },[]);
  function doRest(){
    setChar(c=>({...c,mood:Math.min(100,c.mood+30),hunger:Math.min(100,c.hunger+20)}));
    pushNotif("🍺 溜まり場で休憩！気分UP！","#00e5ff");
  }
  return(<div style={{paddingBottom:80}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",background:"#0a1f1f",borderBottom:"1px solid #1a4a4a",marginBottom:12}}>
      <span style={{color:"#00e5ff",fontWeight:700,fontFamily:"M PLUS Rounded 1c,sans-serif",fontSize:14}}>🍺 {city.name} 溜まり場</span>
    ...
    <div style={{padding:14,background:"#0a1a1a",borderRadius:10,margin:"0 4px 14px",border:"1px solid #1a3a3a",textAlign:"center"}}>
      <div style={{fontSize:40,marginBottom:8}}>🍺</div>
      <div style={{fontSize:11,color:TX2,marginBottom:12,fontFamily:"M PLUS Rounded 1c,sans-serif"}}>ここで仲間を見つけろ。まず休んでいけ。</div>
      <Btn onClick={doRest} col="#0a1f1f" tc="#00e5ff" full sx={{fontSize:12,border:"1px solid #1a4a4a"}}>💤 休憩する（気分+30 お腹+20）</Btn>
    </div>
    <div style={{margin:"0 4px"}}>
      <div style={{fontFamily:"'Press Start 2P',monospace",fontSize:7,color:TX3,marginBottom:10}}>🌐 最近来たダンサー</div>
      {loading&&<div style={{textAlign:"center",color:TX3,fontSize:11,padding:16}}>⏳ 探し中...</div>}
      {!loading&&players.length===0&&<div style={{textAlign:"center",color:TX3,fontSize:11,padding:16,fontFamily:"M PLUS Rounded 1c,sans-serif"}}>まだ誰もいない！最初の仲間になろう。</div>}
      {players.map((p,i)=>(
        <div key={i} style={{background:BG2,border:`1px solid ${BD}`,borderRadius:7,padding:"10px 12px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontWeight:700,fontSize:12,color:TX,fontFamily:"M PLUS Rounded 1c,sans-serif"}}>{GENRES[p.genre]?.e} {p.name}</div>
            <div style={{fontSize:9,color:TX3}}>{GENRES[p.genre]?.jp} · Lv.{p.level} · POWER {p.power}</div>
          </div>
...
/* ── CLUB VIEW (バトル) ── */
function ClubView({city,char,setChar,genre,onBack,pushNotif,addLog}){
  ...
  async function doBattle(){
    ...
    setBattle({phase:"seq",...btl,oppName:city.ch.name,step:0});
    for(let i=0;i<8;i++){await new Promise(r=>setTimeout(r,_battleSpeed===2?400:1100));setBattle(b=>b?{...b,step:Math.min(b.step+1,7)}:null);}
    const{won,flags}=btl;
    const eg=won?city.rw.exp:Math.floor(city.rw.exp*.2);
    const coins=won?city.rw.coins:Math.floor(city.rw.coins*.05);
    const nt=won&&city.rw.title&&!char.titles?.includes(city.rw.title)?city.rw.title:null;
    const gemReward=won&&!cleared[city.id]?3:0;
    ...
    addLog(`${won?"🏆 CLEAR":"💀 敗北"} ${city.name}CLUB +${eg}EXP`);
  }
  return(<div style={{paddingBottom:80}}>
    {battle&&<BattleOverlay state={battle} gc={genre.c} onClose={()=>{setBattle(null);if(battle.phase==="result"&&battle.won)onBack();}}/>}
    ...
    <div style={{textAlign:"center",padding:"24px 16px",background:"#0f0a1a",borderRadius:10,margin:"0 4px 14px",border:"1px solid #2a1a3a"}}>
      <div style={{fontSize:52,marginBottom:10,animation:"fl 2s ease-in-out infinite"}}>{city.ch.e}</div>
      <div style={{fontFamily:"'Press Start 2P',monospace",fontSize:9,color:"#ff4da6",marginBottom:6}}>CLUB BOSS</div>
      <div style={{fontSize:16,color:TX,fontWeight:700,fontFamily:"M PLUS Rounded 1c,sans-serif",marginBottom:4}}>{city.ch.name}</div>
      <div style={{fontSize:11,color:TX3}}>{GENRES[city.g]?.e}{GENRES[city.g]?.jp} · POWER {city.ch.pw.toLocaleString()}</div>
      {isCl&&<div style={{fontSize:11,color:"#60c080",marginTop:8}}>✓ 制覇済み！リターンマッチ可</div>}
    </div>
    {/* 相性表示 */}
    {(()=>{const cb1=compatBonus(char.genre,city.g);const cb2=char.genre2?compatBonus(char.genre2,city.g):{bonus:0};const best=cb2.bonus>cb1.bonus?cb2:cb1;return best.label?(<div style={{textAlign:"center",marginBottom:12,fontSize:11,color:best.col,fontFamily:"M PLUS Rounded 1c,sans-serif",fontWeight:700}}>{GENRES[cb2.bonus>cb1.bonus?char.genre2:char.genre]?.e} {best.label}</div>):null;})()}
    <Btn disabled={char.energy<20} col="#280a0a" tc="#ff7070" onClick={doBattle} full sx={{fontSize:13,padding:"14px",fontWeight:700,border:"1px solid #5a1818"}}>
      {char.energy<20?"⚡ エネルギー不足（20必要）":"⚔️ クラブバトル！ ⚡20"}
    </Btn>
  </div>);
}

/* ── WALKING MAP ── */
function WalkingMap({city,char,setChar,genre,onExit,pushNotif,addLog}){
  const lv=getLv(char.exp);
  const rows=MAP_TEMPLATES[getTemplate(lv)];
  const mapW=rows[0].length;const mapH=rows.length;
  const[pos,setPos]=useState(()=>startPos(rows));
  const[view,setView]=useState("map"); // map|club|inn
  const[prompt,setPrompt]=useState(null);

  function gt(x,y){if(y<0||y>=mapH||x<0||x>=mapW)return"w";return rows[y]?.[x]||"w";}
  function move(dir){
    setPrompt(null);
    setPos(prev=>{
      let{x,y}=prev;let nx=x,ny=y;
      if(dir==="up")ny--;if(dir==="down")ny++;if(dir==="left")nx--;if(dir==="right")nx++;
      const t=gt(nx,ny);const def=TDEFS[t]||TDEFS["."];
      if(def.passable!==false){
        if(def.action)setTimeout(()=>setPrompt({type:def.action,def}),50);
        return{x:nx,y:ny};
      }else{
        if(def.action)setPrompt({type:def.action+"_adj",def});
        return prev;
      }
    });
  }
  ...
  if(view==="club")return<ClubView city={city} char={char} setChar={setChar} genre={genre} onBack={()=>setView("map")} pushNotif={pushNotif} addLog={addLog}/>;
  if(view==="inn")return<InnView city={city} char={char} setChar={setChar} genre={genre} onBack={()=>setView("map")} pushNotif={pushNotif}/>;

  const ts=TILE_SIZE;const svgW=mapW*ts;const svgH=mapH*ts;
  const gc=genre.c;

  return(<div style={{paddingBottom:160}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",background:BG2,borderBottom:`2px solid ${gc}55`,marginBottom:8}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontSize:20}}>{GENRES[city.g]?.e}</span>
        <span style={{color:gc,fontWeight:700,fontSize:15,fontFamily:"M PLUS Rounded 1c,sans-serif"}}>{city.name}</span>
        {char.clearedCities?.[city.id]&&<span style={{fontSize:10,color:"#60c080"}}>✓</span>}
      </div>
      <button onClick={onExit} style={{fontSize:10,color:TX3,background:"none",border:`1px solid ${BD}`,borderRadius:4,padding:"4px 8px",cursor:"pointer",fontFamily:"M PLUS Rounded 1c,sans-serif"}}>🗺 マップへ</button>
    </div>
    {/* タイルマップ */}
    <div style={{border:`1px solid ${BD}`,borderRadius:8,overflow:"hidden",margin:"0 2px 8px"}}>
      <svg viewBox={`0 0 ${svgW} ${svgH}`} width="100%" style={{display:"block"}}>
        ...
        {rows.map((row,y)=>Array.from(row).map((tile,x)=>{
          const def=TDEFS[tile]||TDEFS["."];
          return(<g key={`${x}-${y}`}>
            <rect x={x*ts} y={y*ts} width={ts} height={ts} fill={def.bg||"#14142a"} stroke="#0a0a18" strokeWidth=".5"/>
            {tile!=="w"&&tile!=="."&&def.icon&&<text x={x*ts+ts*.5} y={y*ts+ts*.58} textAnchor="middle" dominantBaseline="middle" fontSize={ts*.65}>{def.icon}</text>}
            {def.label&&<text x={x*ts+ts*.5} y={y*ts+ts*.9} textAnchor="middle" fontSize="3.5" fill={def.col||"#888"} fontFamily="sans-serif">{def.label}</text>}
          </g>);
        }))}
        {/* プレイヤーキャラ */}
        <circle cx={pos.x*ts+ts*.5} cy={pos.y*ts+ts*.5} r={ts*.42} fill={gc} stroke="#fff" strokeWidth="1.5" style={{filter:`drop-shadow(0 0 4px ${gc})`}}/>
        <text x={pos.x*ts+ts*.5} y={pos.y*ts+ts*.6} textAnchor="middle" dominantBaseline="middle" fontSize={ts*.55}>{genre.e}</text>
      </svg>
    </div>
    {/* アクションプロンプト */}
    {prompt&&<div style={{margin:"0 2px 8px",padding:12,background:BG2,borderRadius:8,border:`1px solid ${prompt.def?.col||BD}`,animation:"su .2s ease"}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
        <span style={{fontSize:24}}>{prompt.def?.icon}</span>
        <span style={{color:prompt.def?.col,fontWeight:700,fontSize:13,fontFamily:"M PLUS Rounded 1c,sans-serif"}}>{prompt.def?.label||""}</span>
      </div>
      {(prompt.type==="club"||prompt.type==="club_adj")&&(
        <div>
          <div style={{fontSize:10,color:TX3,marginBottom:8}}>{city.ch.e} {city.ch.name} · POWER {city.ch.pw.toLocaleString()}</div>
          <Btn disabled={char.energy<20} col="#280a0a" tc="#ff7070" onClick={()=>setView("club")} full sx={{fontSize:12,border:"1px solid #5a1818",fontWeight:700}}>{char.energy<20?"⚡ エネルギー不足":"⚔️ クラブに入る！"}</Btn>
        </div>
      )}
      {(prompt.type==="inn"||prompt.type==="inn_adj")&&<Btn col="#0a1f1f" tc="#00e5ff" onClick={()=>setView("inn")} full sx={{fontSize:12,border:"1px solid #1a4a4a"}}>🍺 溜まり場に入る</Btn>}
      {(prompt.type==="exit")&&<Btn col="#0a1428" tc="#80c0ff" onClick={onExit} full sx={{fontSize:12}}>🗺 ワールドマップに戻る</Btn>}
      {(prompt.type==="npc"||prompt.type==="npc_adj")&&<div style={{fontSize:11,color:"#ce93d8",lineHeight:1.7,fontFamily:"M PLUS Rounded 1c,sans-serif"}}>「{city.name}へようこそ！🎵クラブでバトルして名を上げろ。🍺溜まり場で仲間を探せ。POWER {calcPow({...char.stats,...eqBonus(char.equipped||{})})} あれば挑めるはずだ！」</div>}
      {(prompt.type==="shop"||prompt.type==="shop_adj")&&<div style={{fontSize:11,color:"#b3ff00",fontFamily:"M PLUS Rounded 1c,sans-serif"}}>ショップはメニューの「🛍 ショップ」タブから！</div>}
      {(prompt.type==="train"||prompt.type==="train_adj")&&<div style={{fontSize:11,color:"#ffd60a",fontFamily:"M PLUS Rounded 1c,sans-serif"}}>練習はメニューの「🏠 ホーム」タブから！</div>}
    </div>}
    {/* 操作ガイド */}
    <div style={{textAlign:"center",fontSize:9,color:TX3,fontFamily:"M PLUS Rounded 1c,sans-serif",marginBottom:4}}>Dパッドで移動 · 建物に近づいてタップ</div>
    <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:10,fontSize:9,color:TX3}}>
      <span>🎵 CLUB</span><span>🍺 溜まり場</span><span>💬 NPC</span><span>🚪 EXIT</span>
    </div>
    <DPad onMove={move}/>
  </div>);
}
...
function MapTab({char,setChar,genre,pushNotif,addLog}){
  ...
  const[walkingCity,setWalkingCity]=useState(null); // 歩行マップ表示中の都市
  const cleared=char.clearedCities||{};
  ...
  // 歩行マップモード
  if(walkingCity){
    return<WalkingMap city={walkingCity} char={char} setChar={setChar} genre={genre}
      onExit={()=>setWalkingCity(null)} pushNotif={pushNotif} addLog={addLog}/>;
  }
  ...
  return(<div>
    ...
    {curCity&&<div style={{marginBottom:8}}>
      <div style={{fontSize:10,color:TX2,textAlign:"center",marginBottom:6,fontFamily:"M PLUS Rounded 1c,sans-serif"}}>現在地: <span style={{color:GENRES[curCity.g]?.c,fontWeight:700}}>{GENRES[curCity.g]?.e} {curCity.name}</span>{cleared[char.currentCity]&&<span style={{color:TX3}}> ✓</span>}</div>
      <Btn onClick={()=>setWalkingCity(curCity)} col={`${GENRES[curCity.g]?.c}22`} tc={GENRES[curCity.g]?.c} full sx={{fontSize:12,border:`1px solid ${GENRES[curCity.g]?.c}66`,fontWeight:700}}>🚶 {curCity.name}の街を歩く</Btn>
    </div>}
  ...
  async function handleAuth(){
    ...
    try{
      ...
      if(authMode==="register"){
        ...
        // 現在のキャラデータをクラウドに保存
        await saveCharToCloud(u,char);
        setAuthMsg("登録完了！メール確認後ログインできます。データは保存されました！");
      }else{
        u=await signIn(email,pass);
        // ログイン成功 → クラウドにも現在データを保存
        await saveCharToCloud(u,char);
...
/* ── GAME SCREEN ── */
function Game({char,setChar,onTitle,user,onLogout,onAuthChange,manualSave,saveStatus,lightMode,toggleLight}){
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
        ...
        <div style={{textAlign:"right"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:2}}>
            <button onClick={manualSave} style={{fontSize:9,color:saveStatus==="saved"?"#40c060":saveStatus==="saving"?"#ffcc02":saveStatus==="error"?"#ff5555":"#6060a0",background:"none",border:"none",cursor:"pointer",padding:0}}>
...
function LoginScreen({onLogin,onGuest}){
  ...
  async function submit(){
    ...
    try{
      ...
      if(mode==="login"){
        ...
        user=await signUp(email,pass);
        setMsg("確認メールを送りました！メールを確認してからログインしてください。");
        setMode("login");
      }
    }catch(e){
      setErr(e.message==="Invalid login credentials"?"メールかパスワードが違います":e.message);
    }
    setLoading(false);
  }
  ...
  return(
    <div style={{minHeight:"100vh",background:BG,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,fontFamily:"M PLUS Rounded 1c,sans-serif"}}>
      ...
      <div style={{fontFamily:"'Press Start 2P',monospace",fontSize:11,color:"#ff4da6",letterSpacing:3,marginBottom:24}}>DANCER LEGEND</div>
...
export default function DancerLegend(){
  ...
  async function manualSave(){
    ...
    if(user){
      setSaveStatus("saving");
      try{
        await saveCharToCloud(user,char);
        setSaveStatus("saved");
      }catch{setSaveStatus("error");}
    }else{
      setSaveStatus("saved");
  ...
  useEffect(()=>{
    getUser().then(u=>{
      if(u){
        setUser(u);
        loadCharFromCloud(u).then(cloudChar=>{
          if(cloudChar){setCharRaw(migrateChar(cloudChar));setScreen("game");}
          else if(char){setScreen("game");}
          else setScreen("title");
        });
      }else{
        setScreen(char?"title":"login");
      }
    });
  },[]);
  ...
  return(<div style={{background:BG,minHeight:"100vh",width:"100%",maxWidth:"100%",overflowX:"hidden",filter:lightMode?"invert(0.92) hue-rotate(180deg) saturate(0.85)":"none",transition:"filter .3s"}}>
    ...
    {screen==="game"&&char&&<Game char={char} setChar={setChar} onTitle={()=>setScreen("title")} user={user} onLogout={handleLogout} onAuthChange={u=>{setUser(u);if(u)saveCharToCloud(u,char).catch(()=>{});}} manualSave={manualSave} saveStatus={saveStatus} lightMode={lightMode} toggleLight={toggleLight}/>}
  </div>);
}

...
function ZukanTab({genre,char,setChar,pushNotif}){
  ...
  return(<div style={{padding:"0 4px 100px"}}>
    ...
    {/* 歴史の本ボタン */}
    {(()=>{
      const bookOwned=char.bookOwned||false;
      async function buyBook(){
        try{
          const res=await fetch("/api/create-checkout",{
            method:"POST",
            headers:{"Content-Type":"application/json"},
            body:JSON.stringify({type:"book",userId:char.uid||"guest"}),
          });
          const data=await res.json();
          if(data.url){window.location.href=data.url;}
      ...
      return(
        <div style={{background:"linear-gradient(135deg,#2a1500,#1a0c00)",border:"1px solid #c49a3c88",borderRadius:12,padding:"14px 16px",marginBottom:16}}>
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:bookOwned?12:0}}>
            ...
            {bookOwned
              ?<span style={{fontSize:12,color:"#b3ff00",fontFamily:"M PLUS Rounded 1c,sans-serif",fontWeight:700}}>購入済み✅</span>
              :<span style={{fontSize:14,fontWeight:900,color:"#ffd60a",fontFamily:"M PLUS Rounded 1c,sans-serif"}}>¥280</span>
            }
          </div>
          {bookOwned
            ?<div onClick={()=>window.open('/dance_history_encyclopedia.html','_blank')}
               style={{background:"#1a3a00",border:"1px solid #b3ff0044",borderRadius:8,padding:"10px",textAlign:"center",cursor:"pointer",fontSize:12,color:"#b3ff00",fontFamily:"M PLUS Rounded 1c,sans-serif",fontWeight:700}}>
               📖 本を読む →
             </div>
            :<div onClick={buyBook}
               style={{background:"linear-gradient(135deg,#3a2800,#2a1800)",border:"1px solid #c49a3c66",borderRadius:8,padding:"10px",textAlign:"center",cursor:"pointer",fontSize:12,color:"#ffd60a",fontFamily:"M PLUS Rounded 1c,sans-serif",fontWeight:700}}>
               ¥280 で購入して読む →
             </div>
          }
        </div>
...
function Game({char,setChar,onTitle,user,onLogout,onAuthChange,manualSave,saveStatus,lightMode,toggleLight,onTokushou}){
  ...
  useEffect(()=>{
    ...
    const pendingBook=localStorage.getItem("pending_book_notify");
    if(pendingBook){
      localStorage.removeItem("pending_book_notify");
      setTimeout(()=>notif2("📖 ダンスの歴史 大百科を購入しました！図鑑タブから読めます","#ffd60a"),800);
    }
    if(pending){
...
export default function DancingQuest(){
  ...
  useEffect(()=>{
    ...
    if(payment==="book_success"){
      window.history.replaceState({},"",window.location.pathname);
      setTimeout(()=>{
        setChar(c=>{
          if(!c)return c;
          const updated={...c,bookOwned:true};
          localStorage.setItem("dancer_save",JSON.stringify(updated));
          return updated;
        });
        localStorage.setItem("pending_book_notify","1");
      },1500);
    }else if(payment==="success"&&gems>0){
      // URLパラメータをクリア
      window.history.replaceState({},"",window.location.pathname);
